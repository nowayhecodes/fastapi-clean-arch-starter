...
class NotificationRepository(CRUDRepository[Notification, NotificationCreate, NotificationUpdate]):
    ...
    async def get_unread_count(self, db: Session, *, account_id: int) -> int:
        ...
        count = (
            db.query(Notification)
            .filter(Notification.account_id == account_id, Notification.is_read == False)
            .count()
        )
    ...
    async def create(self, db: Session, *, obj_in: NotificationCreate) -> Notification:
        ...
        await cache_manager.clear_pattern(f"{self.cache_prefix}account:{db_obj.account_id}:*")
        await cache_manager.delete(f"{self.cache_prefix}unread:{db_obj.account_id}")
        
        return db_obj

    async def update(self, db: Session, *, db_obj: Notification, obj_in: NotificationUpdate) -> Notification:
    ...
    async def remove(self, db: Session, *, id: int) -> Notification:
        ...
        if notification:
            ...
            # Clear account-specific caches
            await cache_manager.clear_pattern(f"{self.cache_prefix}account:{notification.account_id}:*")
            await cache_manager.delete(f"{self.cache_prefix}unread:{notification.account_id}")
